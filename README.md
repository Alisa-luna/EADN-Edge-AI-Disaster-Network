...
系统综合 **GPS**、**LBS**、**RSSI 测距**、**三边定位** 和 **P 波方位角推算**，实现多节点交叉验证。网关可反向校正节点坐标，服务器支持基于节点观测数据的网关位置一致性校验。
...
6. 上电后节点自动建立 AP (`EQ_Node_ID`, 密码 `12345678`)，手机可连接并访问 `192.168.50.1` 查看状态。网关自动连接 WiFi 或启动 AP (`EQ_Gateway`) 供配置。
